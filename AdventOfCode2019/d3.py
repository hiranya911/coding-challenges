...
def trace_wire(path, start=Point(0, 0)):
    wire = [start]
    for step in path:
        last = wire[-1]
        wire += last.move(step)
...
def find_closest_intersection(path1, path2):
    ...
    print(f'Wire1 length: {len(wire1)}')
    print(f'Wire2 length: {len(wire2)}')
    intersections = set(wire1[1:]).intersection(set(wire2))
    print(f'Intersections: {len(intersections)}')
    port = Point(0, 0)
    return min([p.manhattan_distance(port) for p in intersections])


def step_count(wire, point):
    steps = 0
    for p in wire[1:]:
        steps += 1
        if p == point:
...
def find_min_steps(path1, path2):
    wire1 = trace_wire(path1)
    wire2 = trace_wire(path2)
    print(f'Wire1 length: {len(wire1)}')
    print(f'Wire2 length: {len(wire2)}')
    ...
    for intersection in intersections:
        s1 = wire1.index(intersection)
        s2 = wire2.index(intersection)
        step_counts.append(s1 + s2)
