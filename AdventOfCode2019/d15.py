...
class RepairDroid(d2.InputDevice, d2.OutputDevice):

    def __init__(self):
        self.cells = {(0,0): FREE}
        self.current = (0,0)
        self.next = None
        self.queue = self._next_steps(self.current)
        self.path = [self.current]
        self.goal = None

    def read(self):
        moves = [m for m in self._next_steps(self.current) if m not in self.cells]
        if moves:
            self.next = moves[0]
        else:
            self.path.pop()
            if not self.path:
                self._print_maze()
                raise Exception('Maze explored')
            self.next = self.path.pop()
            
        return self._move_to(self.next)

    def _is_goal_node(self, node):
        return node.child == self.goal

    def bfs(self, init=(0,0), goal_condition=None):
        if goal_condition is None:
            goal_condition = self._is_goal_node
        ...
        while queue:
            ...
            if goal_condition(current):
                return current
            for child in [Node(n, current) for n in self._next_steps(current.child)]:
                if child in explored:
                    continue
                state = self.cells.get(child.child)
                if state is None or state == WALL:
                    continue
                explored.add(child)
                queue.append(child)
    ...
    def fill_with_o2(self):
        init = Node(self.goal, None)
        queue = [[init]]
        filled = {init}
        timer = 0
        while queue:
            ...
            children = []
            for node in current:
                children += [Node(n, node) for n in self._next_steps(node.child)]
    ...
    def _move_to(self, target):
        xdiff = target[0] - self.current[0]
        ydiff = target[1] - self.current[1]
        if xdiff == 0:
            if ydiff == 1:
                return 1
            elif ydiff == -1:
    ...
    def _print_maze(self):
        min_x = min(c[0] for c in self.cells.keys())
        max_x = max(c[0] for c in self.cells.keys())
        min_y = min(c[1] for c in self.cells.keys())
        max_y = max(c[1] for c in self.cells.keys())
        for y in range(max_y, min_y - 1, -1):
            for x in range(min_x, max_x + 1):
                if (x, y) == self.current:
                    print('D', end='')
                    continue
                val = self.cells.get((x, y), -1)
                if val == -1:
                    print(' ', end='')
                elif val == FREE:
                    print('.', end='')
    ...
    def write(self, value):
        if value == 0:
            self.cells[self.next] = WALL
        elif value == 1:
            self.cells[self.next] = FREE
            self.current = self.next
            self.queue += self._next_steps(self.current)
            self.path.append(self.current)
        else:
            self.cells[self.next] = GOAL
            self.goal = self.next
            self.current = self.next
            self.path.append(self.current)


class DroidIOManager(d2.IOManager):

    def __init__(self):
        self.droid = RepairDroid()
...
print()
print(io.droid.distance_to_goal())
print(io.droid.fill_with_o2())
